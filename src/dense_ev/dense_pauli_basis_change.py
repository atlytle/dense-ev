...
class DensePauliBasisChange(ConverterBase):
    r"""
    ...
    def convert(self, operator: OperatorBase) -> OperatorBase:
        ...
        if (
            ...
            PO = PauliOrganizer(m)
            fams = PO.f
            _id = PO.get_fam_index(str(origin_pauli))
            ...
            logger.debug(f"cob_instr_op: {cob_instr_op}")
    ...
    def do_nothing(self, pauli: Union[Pauli, PauliOp]) -> OperatorBase:
        if isinstance(pauli, PauliOp):
            pauli = pauli.primitive
        ...
        y_to_x_origin = tensorall(
            [I if has_y else I for has_y in reversed(np.logical_and(pauli.x, pauli.z))]
        ).adjoint()
        x_to_z_origin = tensorall([I if has_x else I for has_x in reversed(pauli.x)])
        return x_to_z_origin.compose(y_to_x_origin)
    ...
    def get_cob_circuit(self, origin: Union[Pauli, PauliOp]) -> Tuple[PrimitiveOp, PauliOp]:
        ...
        """
        logger.debug("Called get_cob_circuit()...")
    ...
    def get_new_cob_circuit(self, origin: Union[Pauli, PauliOp]) -> Tuple[PrimitiveOp, PauliOp]:
        ...
        """
        logger.debug("Called get_new_cob_circuit()...")
        # If pauli is an PrimitiveOp, extract the Pauli
        if isinstance(origin, Pauli):
            origin = PauliOp(origin)
        ...
        PO = PauliOrganizer(m)
        fams = PO.f
        logger.debug(PO.get_family_of_string(str(origin)).to_string())
        _id = PO.get_fam_index(str(origin))
        logger.debug(f"{_id = }")
        cob_instruction = self.do_nothing(origin)
        logger.info(f"cob_instruction: {cob_instruction}")
        _cob_instruction = cob_instruction.to_circuit().copy()
        logger.info(f"_cob_instruction (before): {_cob_instruction}")
        fams[_id].apply_to_circuit(_cob_instruction)
        _cob_instruction = CircuitOp(_cob_instruction)
        logger.info(f"_cob_instruction (after): {_cob_instruction}")

        return cast(PrimitiveOp, _cob_instruction), destination
